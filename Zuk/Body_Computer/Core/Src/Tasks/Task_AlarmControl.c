...
void StartTaskAlarmControl(void const * argument)
{
	...
	for (;;)
	{
		...
		if(((ENC_button.longPressDetectedBuzzer) && (BUZZER_settings.buzzerWhenLognPressOn)) ||
				((ENC_button.shortPressDetectedBuzzer) && (BUZZER_settings.buzzerWhenShortPressOn)))
		{
			...
			ENC_button.allFlags &= 0b11100111;	/* Clear the button alarms bits (long and short press buzzer) */
		}
