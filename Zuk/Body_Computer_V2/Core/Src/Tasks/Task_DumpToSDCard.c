...
static uint8_t CSVHeadersTable[MAX_LENGTH_CSV_HEADER_TABLE] =
		"Time,Latitude,LatitudeIndicator,Longitude,LongitudeIndicator,Altitude,SpeedByGPS,SatelitesUsed,Speed,CruiseControlOn,SetSpeed,CoolantTemp\n\r";
...
static void prepareDataToSave(uint16_t* dataBufferPosition)
{
	...
	{
		(void)copy_str_to_buffer((char*)GPS.forLCD.clock.messageHandler, (char*)CSVDataTable, *dataBufferPosition, GPS.forLCD.clock.size);
	...
	{
		(void)copy_str_to_buffer((char*)GPS.forLCD.latitude.messageHandler, (char*)CSVDataTable, *dataBufferPosition, GPS.forLCD.latitude.size);
	...
	{
		(void)copy_str_to_buffer((char*)GPS.forLCD.latitudeIndicator.messageHandler, (char*)CSVDataTable, *dataBufferPosition, GPS.forLCD.latitudeIndicator.size);
		*dataBufferPosition += GPS.forLCD.latitudeIndicator.size;
	}
	addCSVDelimiter(CSVDataTable, dataBufferPosition);
	if(TRUE == GPS.forLCD.longitude.messageReadyFLAG) /* Saving longitude */
	{
		(void)copy_str_to_buffer((char*)GPS.forLCD.longitude.messageHandler, (char*)CSVDataTable, *dataBufferPosition, GPS.forLCD.longitude.size);
	...
	{
		(void)copy_str_to_buffer((char*)GPS.forLCD.longitudeIndicator.messageHandler, (char*)CSVDataTable, *dataBufferPosition, GPS.forLCD.longitudeIndicator.size);
		*dataBufferPosition += GPS.forLCD.longitudeIndicator.size;
	}
	addCSVDelimiter(CSVDataTable, dataBufferPosition);
	if(TRUE == GPS.forLCD.altitude.messageReadyFLAG) /* Saving altitude */
	{
		(void)copy_str_to_buffer((char*)GPS.forLCD.altitude.messageHandler, (char*)CSVDataTable, *dataBufferPosition, GPS.forLCD.altitude.size);
		*dataBufferPosition += GPS.forLCD.altitude.size;
	}
	addCSVDelimiter(CSVDataTable, dataBufferPosition);
	if(TRUE == GPS.forLCD.speed.messageReadyFLAG) /* Saving speed from GPS */
	{
		(void)copy_str_to_buffer((char*)GPS.forLCD.speed.messageHandler, (char*)CSVDataTable, *dataBufferPosition, GPS.forLCD.speed.size);
	...
	{
		(void)copy_str_to_buffer((char*)GPS.forLCD.satellitesUsed.messageHandler, (char*)CSVDataTable, *dataBufferPosition, GPS.forLCD.satellitesUsed.size);
	...
	{
		(void)copy_str_to_buffer((char*)SPEEDForLCD.messageHandler, (char*)CSVDataTable, *dataBufferPosition, SPEEDForLCD.size);
	...
	if(TRUE == Wanted_SPEEDForLCD.messageReadyFLAG) /* Saving speed calculated from a sensor */
	{
		...
		*dataBufferPosition += Wanted_SPEEDForLCD.size;
	}
	addCSVDelimiter(CSVDataTable, dataBufferPosition);
	if(TRUE == waterTemperatureValueForLCD.messageReadyFLAG) /* Saving coolant temperature */
	{
		(void)copy_str_to_buffer((char*)waterTemperatureValueForLCD.messageHandler, (char*)CSVDataTable, *dataBufferPosition, waterTemperatureValueForLCD.size);
